...
// Retrieve all tasks for the user (completed + uncompleted)
$sql = "SELECT * FROM tasks WHERE userid = ? AND is_deleted = 0 AND projectid IS NULL";
...
<head>
    ...
    <style>
        /* Your CSS styling here (unchanged) */
        body {
...
<div class="container">
    ...
    <div class="filter-section">
        ...
        <button class="filter-btn" data-filter="overdue">Overdue Tasks</button>
    </div>
    ...
    <?php
        if ($result && mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                $isCompleted = ($row['taskstatus'] == 'completed');
                $isOverdue = (!$isCompleted && $taskDateTime < $currentDateTime);
        ...
        button.addEventListener('click', () => {
            ...
            taskBoxes.forEach(box => {
                if (filter === 'all') {
                    ...
                    box.style.display = box.classList.contains('completed-task') ? 'flex' : 'none';
                } else if (filter === 'overdue') {
                    box.style.display = box.classList.contains('overdue-task') ? 'flex' : 'none';
                }
            });
        });
    });
